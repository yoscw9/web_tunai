...
class ToppingsController extends Controller
{
    ...
    {
        $topping = Toppings::latest()->paginate(5);

        return view('pages.menus.topping', compact('topping'));
    }
     ...
     */
    public function edit(Request $request, Toppings $toppings)
    {
        $jenis = $request->segment(2);

        $title = 'Edit ' . ucfirst($jenis);
        $url = route($jenis . '.update', $toppings->id);
        $form = collect([
            [
                'type' => 'text',
                'name' => 'name',
     ...
     */
    public function destroy(Request $request, Toppings $toppings)
    {
        $toppings->delete();
