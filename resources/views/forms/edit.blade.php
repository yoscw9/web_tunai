                ...
                @foreach ($form as $input)
                    <div class="row mb-3">
                        ...
                        @switch($input['type'])
                            @case('select')
                                <select name="{{ $input['name'] }}" class="form-control">
                                    @foreach ($input['options'] as $id => $name)
                                        <option value="{{ $id }}" {{ $input['value'] == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            @break
                            @case('file')
                            ...
                            @break
                            @default
                            <input type="{{ $input['type'] }}" name="{{ $input['name'] }}" class="form-control" placeholder="{{ $input['placeholder'] }}" value="{{ isset($input['value']) ? $input['value'] : old($input['name']) }}" autocomplete="off" />
                        @endswitch
                        </div>
                    </div>
                @endforeach
